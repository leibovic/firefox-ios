...
@interface SSReadingList (Firefox)
+ (BOOL)alwaysFailingSupportsURL:(NSURL *)URL;
@end

@implementation SSReadingList (Firefox)
+ (void) initialize {
    SwizzleClassMethods([SSReadingList class], @selector(supportsURL:), @selector(alwaysFailingSupportsURL:));
}
+ (BOOL)alwaysFailingSupportsURL:(NSURL *)URL {
    return NO;
}
